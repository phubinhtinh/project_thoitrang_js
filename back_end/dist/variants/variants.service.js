...
let VariantsService = class VariantsService {
    ...
    async create(colorId, dto) {
        const color = await this.prisma.productColor.findUnique({ where: { id: colorId } });
        if (!color)
            throw new common_1.NotFoundException('Màu sản phẩm không tồn tại');
    ...
    async remove(id) {
        ...
        if (count <= 1) {
            throw new common_1.BadRequestException('Không thể xóa size cuối cùng của màu này. Hãy xóa cả màu nếu không cần.');
        }
