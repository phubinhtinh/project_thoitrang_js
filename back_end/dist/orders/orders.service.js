    ...
    async checkout(userId, dto) {
        ...
        for (const item of cartItems) {
            if (item.variant.stockQuantity < item.quantity) {
                throw new common_1.BadRequestException(`Sản phẩm "${item.variant.sku}" (${item.variant.productColor.color} - ${item.variant.size}) chỉ còn ${item.variant.stockQuantity} sản phẩm trong kho`);
            }
        }
