...
/**
 * Seeder sản phẩm — dữ liệu khớp theo từng leaf category.
 * Chạy: npx ts-node prisma/seed-products.ts
 * Hoặc:  npx prisma db seed --preview-feature
 */

type Variant = { size: string; color: string; stockQuantity: number; sku: string };
type ProductSeed = {
  name: string;
  description: string;
  basePrice: number;
  ...
  img: string;
  /** Map ảnh cho từng màu. Nếu không có, dùng `img` chung của product. */
  colorImages?: Record<string, string>;
  variants: Variant[];
};

// Map theo TÊN danh mục (case-insensitive), không phụ thuộc vào ID
...
// Cây danh mục (parent → children) — tự động tạo nếu chưa có
const CATEGORY_TREE: { name: string; description: string; children: { name: string; description: string }[] }[] = [
  {
    name: 'Thời Trang Nữ',
...
async function main() {
  ...
  for (const c of categories) catMap.set(c.name.toLowerCase().trim(), c.id);
